...
| D-17 | Los encabezados de las tres tablas son **etiquetas legibles** —"Cancha", "Nombre", "Deporte", "Horas reservadas", "Horas disponibles", "Ocupacion (%)", "Reservas", "Cancelaciones"—, con los nombres de campo del JSON intactos en el código | Usar el nombre del campo como encabezado (`canchaId`, `porcentajeOcupacion`, `totalCancelaciones`), por fidelidad literal al contrato | El contrato congela los **nombres de campo** del JSON, no el texto de un encabezado de tabla: `item.canchaId` y `item.porcentajeOcupacion` se leen y se envían con su nombre exacto, y eso es lo que §1 verifica. Un encabezado que dice `totalCancelaciones` no hace más fiel la integración; solo obliga al administrador a leer camelCase en una pantalla de solo lectura. Las notas al pie sí siguen citando el campo por su nombre (`horasDisponibles`, `totalReservas`), porque ahí lo que se explica es la regla del contrato. Precisión acordada el 25/08/2026, tras T6 |
| D-18 | `.mfrep-rango` declara `flex-direction: row` **explícito** | Dejar solo `display: flex`, como en el `.mfr-filtros` de `mf-reservas` | El contenedor del rango es un `<form>`, y el CSS del shell declara `form { display: flex; flex-direction: column }`. La clase gana en `display` pero no declaraba dirección, así que la ponía el shell: los tres controles salían en columna y, con `align-items: flex-end`, pegados a la derecha con un hueco a la izquierda. `mf-reservas` no lo sufre porque su fila de filtros es un `div`. Cuarto caso del proyecto de una regla global del shell filtrándose en un remote (hallazgo 3 de la spec 08) |
